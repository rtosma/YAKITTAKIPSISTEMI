...
technical.push(stat('Backend hedefleri ayakta (min up)', 'min(up{job="backend"})', grid(0, y, 5, 4), { thresholds: [{ color: 'red', value: null }, { color: 'green', value: 1 }], description: '1 = tüm backend replikaları kazınıyor; 0 = en az biri erişilemiyor.' }));
technical.push(stat('5xx oranı (5 dk)', '(sum(rate(http_requests_total{status=~"5.."}[5m])) or vector(0)) / clamp_min(sum(rate(http_requests_total[5m])), 1e-9)', grid(5, y, 5, 4), { unit: 'percentunit', decimals: 2, thresholds: amber(0.01, 0.05) }));
technical.push(stat('İstek / sn', 'sum(rate(http_requests_total[1m]))', grid(10, y, 5, 4), { decimals: 2 }));
technical.push(stat('İşlenen istek (anlık)', 'sum(http_requests_in_flight)', grid(15, y, 4, 4), { thresholds: amber(50, 200) }));
technical.push(stat('Ateşleyen uyarı (critical/warning)', 'count(ALERTS{alertstate="firing", severity=~"critical|warning"}) or vector(0)', grid(19, y, 5, 4), { thresholds: [{ color: 'green', value: null }, { color: 'orange', value: 1 }, { color: 'red', value: 3 }], description: 'Prometheus\'ta ateşleyen uyarılar (docs/ALERTING.md). Watchdog ve info sayılmaz.' }));
y += 4;
