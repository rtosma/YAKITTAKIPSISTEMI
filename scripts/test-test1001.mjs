...
check('Paralellik: E2E dosyaları Promise.all ile EŞZAMANLI koşar; imaj derlemeleri ve bağımlılık başlatmaları da paralel; hiçbir konteyner sabit host portu yayınlamaz (-p yok → paralel koşular/CI çakışmaz)', /Promise\.all\(files\.map/.test(orch) && /const \[a, b\] = await Promise\.all\(\[/.test(orch) && !/'-p'/.test(orch) && !/publish/.test(orch), '');
